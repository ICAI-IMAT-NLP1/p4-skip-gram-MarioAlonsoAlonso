...
class SkipGramNeg(nn.Module):
    ...
    def forward_noise(self, batch_size: int, n_samples: int) -> torch.Tensor:
        ...
        # TODO
        noise_words = torch.randint(0, len(noise_dist), (n_samples * batch_size,))
...
class NegativeSamplingLoss(nn.Module):
    ...
    def forward(self, input_vectors: torch.Tensor, output_vectors: torch.Tensor,
        ...
        positive_dot_product = torch.bmm(input_vectors.unsqueeze(1), output_vectors.unsqueeze(2)).squeeze()
        out_loss = torch.nn.functional.logsigmoid(positive_dot_product)
        negative_dot_product = torch.bmm(noise_vectors, input_vectors.unsqueeze(2)).squeeze()
        noise_loss = torch.nn.functional.logsigmoid(-negative_dot_product).sum(dim=1)
